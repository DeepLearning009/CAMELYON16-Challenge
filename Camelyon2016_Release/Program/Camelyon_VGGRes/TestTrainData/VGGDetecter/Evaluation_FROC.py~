...
def plotFROC(total_FPs, total_sensitivity, figPath):
    ...
    fig.suptitle('Free response receiver operating characteristic curve', fontsize=12)
    plt.plot(total_FPs, total_sensitivity, '-', color='#000000')
    plt.savefig(result_folder+'/'+'Figure2.png')    
    plt.show()     

def findPosition(total_FPs, value):      
    diff = 1000000.0
    Position_ID = -1
    for i in xrange(len(total_FPs)):
...
if __name__ == "__main__":
    ...
    mask_folder = "../../../2016ISBI/CAMELYON16/TrainingData/Ground_Truth/Mask"
    result_folder = "./VGGResults"
    figName = "Figure.png"
    txtName = "Figure.txt"
