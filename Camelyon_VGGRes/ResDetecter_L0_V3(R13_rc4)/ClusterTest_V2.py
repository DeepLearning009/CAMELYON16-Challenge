...
def postProcessing(csvPath, outputDir, step_size = 227, drop_size = 4):
    file = open( csvPath,'r')
    ...
    for i in xrange(len(coor_list)):   
        ...
        if prob_list[i] > 0.5 and iso_flag == False:
            t_problist.sort()
            probM[hcoor_t,wcoor_t] = (t_problist[8]+t_problist[7])/2
            #probM[hcoor_t,wcoor_t] = prob_list[i]
        if t_tmp[hcoor_t, wcoor_t] == 0:
            probM[hcoor_t,wcoor_t] = 0
...
if __name__ == '__main__':
    ...
    for i in xrange(1,111):
        if i not in Ex_list :
            ...
            csvPath = "./Test/Tumor_%03d.csv" %i
            postProcessing(csvPath, outputDir, 227, 0)
    print "Finish!"
