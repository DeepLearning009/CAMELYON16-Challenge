        ...
        if os.path.exists(self._outputDir + "/" +dataName+"_coor.txt"):
            ...
            while(w<zero_level_size[0] and stop_flag_1 == False):
                ...
                while(h<zero_level_size[1] and stop_flag_2 == False):
                    ...
                    OVslideTile = cv2.resize(OVslideTile,(227,227)).astype('float32')
                    
                    OVslideTile3 = self._GetPatch(slide, w, h, (OVwindowShape[0]*8,OVwindowShape[1]*8), self._workingLevel)           
                    r3,g3,b3,a3 = cv2.split(OVslideTile3)
                    OVslideTile3 = cv2.merge([r3,g3,b3])
                    OVslideTile3 = cv2.resize(OVslideTile3,(227,227)).astype('float32')
                    
                    OVmaskTile4 = self._GetPatch(mask, w, h, (OVwindowShape[0]*8,OVwindowShape[1]*8), self._workingLevel)               
                    r2,g2,b2,a2 = cv2.split(OVmaskTile4)
                    OVmaskTile4 = cv2.merge([r2])  
                    
                    OVmaskTile = self._GetPatch(mask, w, h, OVwindowShape, self._workingLevel)               
                    r4,g4,b4,a4 = cv2.split(OVmaskTile)
                    OVmaskTile = cv2.merge([r4])  
                    ...
                    if(t_tissue.sum()>5 and white_flag == False ):
                        ...
                        if OVmaskTile.max()>100:
                            cv2.imwrite("/home/hjlin/Desktop/TempResult/OV_FN.jpg",OVslideTile)
                            cv2.imwrite("/home/hjlin/Desktop/TempResult/OV_FN_mask.jpg",OVmaskTile)
                            cv2.imwrite("/home/hjlin/Desktop/TempResult/OV_FN_mask_L5.jpg",OVmaskTile4)
                            cv2.imwrite("/home/hjlin/Desktop/TempResult/OV_FN_L5.jpg",OVslideTile3)
                            print "I am here!!!"
            ...
            for i in xrange(len(OV_node_list)):
                ...
                while(w < W_Bond_Max and stop_flag_1 == False):
                    ...
                    while(h < H_Bond_Max and stop_flag_2 == False):
                        ...
                        slideTile = cv2.resize(slideTile,(227,227)).astype('float32')
...
if __name__ == '__main__':
    outputDir = "/home/hjlin/Desktop/TempResult"
    
    txtMaker = CrossTxtMaker()
    txtMaker.SetWorkingLevel(1)
    txtMaker.SetStepSize(113)
